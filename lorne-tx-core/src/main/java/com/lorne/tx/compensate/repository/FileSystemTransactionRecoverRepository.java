 ...
 */
@Component
public class FileSystemTransactionRecoverRepository implements TransactionRecoverRepository {
    ...
    private String getFullFileName(String id) {
        return String.format("%s/%s", rootPath, id);
    }
    ...
    private void makeDirIfNecessory() {
        if (!initialized) {
            synchronized (FileSystemTransactionRecoverRepository.class) {
                if (!initialized) {
                    ...
                    if (!rootPathFile.exists()) {
                        ...
                        if (!result) {
                            throw new TransactionRuntimeException("cannot create root path, the path to create is:" + rootPath);
                        }
